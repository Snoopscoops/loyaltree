...
function priceFor(planData, branchCount, billingCycle = 'monthly') {
  ...
  if (billingCycle === 'annual') {
    return planData.price_tiers_annual?.[bracket]
      ?? ((planData.price_tiers?.[bracket] ?? planData.price_month) * 10)
  }
...
function SignaturePad({ canvasRef, hasInk, setHasInk }) {
  const drawingRef = useRef(false)
  const pointerIdRef = useRef(null)
  const inkRef = useRef(Boolean(hasInk))
  inkRef.current = Boolean(hasInk)
  ...
  const move = (e) => {
    ...
    // The first actual drawn segment is enough to make the signature valid.
    // This does not redraw, resize, replace, or clear the canvas.
    if (!inkRef.current) {
      inkRef.current = true
      setHasInk(true)
    }
  ...
  const end = (e) => {
    ...
    // Intentionally NO clearRect, width/height reset, state-to-canvas redraw,
    // or other operation here. Releasing the mouse must leave the ink intact.
  }
  ...
  return <div style={styles.signatureWrap}>
    ...
    <div style={styles.signatureHintRow}>
      <span style={styles.tip}>Draw normally, then release the mouse. Your signature will stay. You can add as many strokes as you need.</span>
      <button type="button" onPointerDown={e=>e.stopPropagation()} onClick={clear} style={styles.clearSignature}>Clear signature</button>
    </div>
  </div>
}
...
function Signup({ API_BASE }) {
  ...
  const createAccount=async()=>{
    ...
    const capturedSignature = signatureCanvasRef.current?.toDataURL('image/png') || agreement.signature_data_url
  ...
  return <div style={styles.page}><style>{`@media(max-width:720px){.lt-signup-shell{grid-template-columns:1fr!important}.lt-signup-side{display:none!important}.lt-signup-main{padding:28px 20px!important}.lt-signup-two{grid-template-columns:1fr!important}.lt-signup-plans{grid-template-columns:1fr!important}.lt-agreement-summary{grid-template-columns:1fr!important}}`}</style><div className="lt-signup-shell" style={styles.shell}>
    ...
    <main className="lt-signup-main" style={styles.main}>
      ...
      {wizardStep===4&&<section><p style={styles.eyebrow}>4 · PLAN + PR KIT</p><h1 style={styles.title}>Choose how you’ll launch</h1><p style={styles.subtitle}>Choose monthly or annual billing. Annual gives 12 months of access for the price of 10. Your exact plan, branch count, billing cycle, subscription price, and optional PR Kit total will appear in the agreement you sign next.</p>
        ...
        <div style={styles.priceDisclosure}><strong>Agreement preview</strong><span>Billing: {form.billing_cycle==='annual'?'Annual · 12 months for the price of 10':'Monthly · 30-day period'}</span><span>Subscription: ₱{Number(selectedPrice||0).toLocaleString()}{form.billing_cycle==='annual'?'/year':'/30 days'}</span>{kitTotal>0&&<span>PR Kit: ₱{kitTotal.toLocaleString()} one-time</span>}</div>
      </section>}
        ...
        {!agreementLoading&&agreementDoc&&<>
          ...
          <div style={styles.signerBox}>
            ...
            <div style={styles.field}>
              ...
              <SignaturePad key={agreementDoc.agreement_sha256} canvasRef={signatureCanvasRef} hasInk={signatureHasInk} setHasInk={setSignatureHasInk}/>
      ...
      {wizardStep===6&&<section><p style={styles.eyebrow}>6 · PAYMENT</p><h1 style={styles.title}>Activate your business</h1><p style={styles.subtitle}>Your signed agreement has been recorded. Complete your payment to activate the business, then sign in and continue the LoyaltyTree onboarding guide.</p>
        {loading?<div style={styles.loadingBox}>Creating your business account…</div>:businessSlug?<SubscriptionPayment API_BASE={API_BASE} businessSlug={businessSlug} initialBillingCycle={form.billing_cycle} title="Pay & Activate" subtitle={form.setup_kit_requested?`Your total includes your ${form.billing_cycle} subscription plus ₱${kitTotal.toLocaleString()} for the PR Kit (${branchCount} × ₱150).`:`Pay your selected ${form.billing_cycle} plan to activate your business.`} successMessage="🎉 Payment received — your business is active!" onPaid={()=>{localStorage.setItem('loyaltree_continue_onboarding','1');navigate('/login?onboarding=1')}}/>:<div style={styles.error}>The account was not created. Go back to Review + Sign and try again.</div>}
      </section>}
