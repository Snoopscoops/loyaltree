...
function StaffManagement({API_BASE,session}){
  const empty={full_name:'',username:'',password:'',email:'',role:session.staff.role==='regional'?'local':'regional',region_public_id:session.staff.region_public_id||'',branch_public_id:'',branch_name:''}
  const [staff,setStaff]=useState([]),[regions,setRegions]=useState([]),[branches,setBranches]=useState([]),[error,setError]=useState(''),[show,setShow]=useState(false),[editing,setEditing]=useState(null)
  const [f,setF]=useState(empty)
  const auth={'Authorization':`Bearer ${session.token}`,'Content-Type':'application/json'}
  async function load(){try{
    if(session.staff.role==='national'){
      const boot=await fetch(`${API_BASE}/api/v1/motolite/regions/bootstrap-philippines`,{method:'POST',headers:auth});const bd=await boot.json();if(!boot.ok)throw new Error(bd.detail||'Could not initialize Philippine regions')
      const clean=await fetch(`${API_BASE}/api/v1/motolite/branches/dedupe`,{method:'POST',headers:auth});const cd=await clean.json();if(!clean.ok)throw new Error(cd.detail||'Could not clean duplicate branches')
    }
    const [a,r,b]=await Promise.all([fetch(`${API_BASE}/api/v1/motolite/staff`,{headers:auth}),fetch(`${API_BASE}/api/v1/motolite/regions`),fetch(`${API_BASE}/api/v1/motolite/branches`)]);const ad=await a.json();if(!a.ok)throw new Error(ad.detail||'Could not load staff');setStaff(ad);setRegions(await r.json());setBranches(await b.json())
  }catch(e){setError(e.message)}}
  React.useEffect(()=>{load()},[])
  const set=(k,v)=>setF(x=>({...x,[k]:v,...(k==='region_public_id'?{branch_public_id:'',branch_name:''}:{})}))
  function beginCreate(){setEditing(null);setF(empty);setShow(true)}
  function beginEdit(x){const currentBranch=branches.find(b=>b.public_id===x.branch_public_id);setEditing(x.public_id);setF({full_name:x.full_name||'',username:x.username||'',password:'',email:x.email||'',role:x.role,region_public_id:x.region_public_id||'',branch_public_id:x.branch_public_id||'',branch_name:currentBranch?.name||''});setShow(true)}
  async function saveStaff(){setError('');const typedBranch=(f.branch_name||'').trim();const exact=allowedBranches.find(b=>(b.name||'').trim().toLowerCase()===typedBranch.toLowerCase());const body={...f,region_public_id:f.role==='national'?null:(f.region_public_id||null),branch_public_id:f.role==='local'?(exact?.public_id||null):null,branch_name:f.role==='local'?typedBranch:null};if(editing)delete body.password;const r=await fetch(editing?`${API_BASE}/api/v1/motolite/staff/${editing}`:`${API_BASE}/api/v1/motolite/staff`,{method:editing?'PUT':'POST',headers:auth,body:JSON.stringify(body)});const d=await r.json();if(!r.ok){setError(d.detail||`Could not ${editing?'update':'create'} account`);return}setShow(false);setEditing(null);setF(empty);load()}
  ...
  return <section className="staffPanel"><div className="staffHead"><div><h2>Staff Accounts</h2><p>{session.staff.role==='national'?'Create and edit National, Regional and Local accounts.':'Create and manage Local accounts in your region.'}</p></div><button className="redBtn" onClick={beginCreate}>+ Create Account</button></div>{error&&<div className="formError">{error}</div>}{show&&<div className="staffCreate"><h3>{editing?'Edit Staff Account':'Create Staff Account'}</h3><div className="formGrid"><Field label="Full Name" value={f.full_name} setValue={v=>set('full_name',v)} required/><Field label="Username" value={f.username} setValue={v=>set('username',v)} required/>{!editing&&<Field label="Temporary Password" value={f.password} setValue={v=>set('password',v)} required/>}<Field label="Email" value={f.email} setValue={v=>set('email',v)}/><label className="field"><span>Role *</span><select value={f.role} onChange={e=>set('role',e.target.value)}>{session.staff.role==='national'&&<option value="national">National</option>}{session.staff.role==='national'&&<option value="regional">Regional</option>}<option value="local">Local</option></select></label>{f.role!=='national'&&<label className="field"><span>Region *</span><select value={f.region_public_id} onChange={e=>set('region_public_id',e.target.value)} disabled={session.staff.role==='regional'}><option value="">Select region</option>{regions.map(r=><option key={r.public_id} value={r.public_id}>{r.name}</option>)}</select></label>}{f.role==='local'&&<label className="field"><span>Branch name *</span><input list="motolite-branch-suggestions" value={f.branch_name||''} onChange={e=>setF(x=>({...x,branch_name:e.target.value,branch_public_id:''}))} placeholder="Type branch name"/><datalist id="motolite-branch-suggestions">{allowedBranches.slice(0,250).map(b=><option key={b.public_id} value={b.name}/>)}</datalist><small style={{color:'#7b8088',fontSize:11,lineHeight:1.45}}>Type the branch name. Existing names are reused; a new branch is created if it does not exist in the selected region.</small></label>}</div><div className="formActions"><button className="ghostBtn" onClick={()=>{setShow(false);setEditing(null)}}>Cancel</button><button className="redBtn" disabled={!f.full_name||!f.username||(!editing&&!f.password)||(f.role!=='national'&&!f.region_public_id)||(f.role==='local'&&!String(f.branch_name||'').trim())} onClick={saveStaff}>{editing?'Save Changes':'Create Staff Account'}</button></div></div>}<div className="staffTable"><div className="staffRow staffHeader"><b>Name</b><b>Username</b><b>Role</b><b>Status</b><b>Action</b></div>{staff.map(x=><div className="staffRow" key={x.public_id}><span>{x.full_name||'—'}</span><span>{x.username}</span><span className="rolePill">{x.role}</span><span>{x.is_active?'Active':'Disabled'}</span><div className="staffActions">{session.staff.role==='national'&&<button onClick={()=>beginEdit(x)}>Edit</button>}<button onClick={()=>toggle(x)}>{x.is_active?'Disable':'Enable'}</button></div></div>)}</div></section>
}
