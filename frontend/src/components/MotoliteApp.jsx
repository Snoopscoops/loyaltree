...
function StaffManagement({API_BASE,session}){
  const empty={full_name:'',username:'',password:'',email:'',role:session.staff.role==='regional'?'local':'regional',region_public_id:session.staff.region_public_id||'',branch_public_id:''}
  ...
  async function load(){try{
    if(session.staff.role==='national'){const boot=await fetch(`${API_BASE}/api/v1/motolite/regions/bootstrap-philippines`,{method:'POST',headers:auth});const bd=await boot.json();if(!boot.ok)throw new Error(bd.detail||'Could not initialize Philippine regions')}
  ...
  const set=(k,v)=>setF(x=>({...x,[k]:v,...(k==='region_public_id'?{branch_public_id:''}:{})}))
  function beginCreate(){setEditing(null);setF(empty);setShow(true)}
  function beginEdit(x){setEditing(x.public_id);setF({full_name:x.full_name||'',username:x.username||'',password:'',email:x.email||'',role:x.role,region_public_id:x.region_public_id||'',branch_public_id:x.branch_public_id||''});setShow(true)}
  async function saveStaff(){setError('');const body={...f,region_public_id:f.role==='national'?null:(f.region_public_id||null),branch_public_id:f.role==='local'?(f.branch_public_id||null):null};if(editing)delete body.password;const r=await fetch(editing?`${API_BASE}/api/v1/motolite/staff/${editing}`:`${API_BASE}/api/v1/motolite/staff`,{method:editing?'PUT':'POST',headers:auth,body:JSON.stringify(body)});const d=await r.json();if(!r.ok){setError(d.detail||`Could not ${editing?'update':'create'} account`);return}setShow(false);setEditing(null);setF(empty);load()}
