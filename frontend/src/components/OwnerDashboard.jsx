...
function OrderAheadMenuBuilder({ API_BASE, user, business, branches, authFetch, onChanged, setMessage }) {
  ...
  const emptyItem={name:'',description:'',category_public_id:'',base_price:'',image_url:'',is_active:true,is_available:true,modifier_group_public_ids:[],unavailable_branch_public_ids:[]}
  const [itemForm,setItemForm]=useState(emptyItem)
  const [editingItem,setEditingItem]=useState(null)
  const [categoryForm,setCategoryForm]=useState({name:'',description:'',is_active:true})
  const [editingCategory,setEditingCategory]=useState(null)
  const [groupForm,setGroupForm]=useState({name:'',selection_type:'single',is_required:false,min_selections:0,max_selections:1,is_active:true,options:[{name:'',price_delta:0,is_active:true}]})
  ...
  const beverageGroupIds=()=>data.modifier_groups.filter(g=>['size','sugar level','sugar'].includes(String(g.name||'').trim().toLowerCase())).map(g=>g.public_id)
  const categoryLooksLikeBeverage=(categoryPublicId)=>{const name=String(data.categories.find(c=>c.public_id===categoryPublicId)?.name||'').toLowerCase();return ['coffee','non-coffee','non coffee','drink','beverage','tea'].some(k=>name.includes(k))}
  const chooseItemCategory=(categoryPublicId)=>{setItemForm(f=>{const current=f.modifier_group_public_ids||[];const defaults=categoryLooksLikeBeverage(categoryPublicId)?beverageGroupIds():[];return {...f,category_public_id:categoryPublicId,modifier_group_public_ids:[...new Set([...current,...defaults])]}})}
  const quickSetupBeverageModifiers=async()=>{setSaving(true);try{const r=await authFetch(`${API_BASE}/api/v1/business/${slug}/order-ahead/beverage-modifiers/quick-setup`,{method:'POST'});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.detail||'Could not set up beverage options');await loadMenu();onChanged?.();setMessage?.(`Size + Sugar Level ready · ${j.beverage_items_found||0} beverage item${Number(j.beverage_items_found||0)===1?'':'s'} updated`)}catch(e){setMessage?.(e.message||'Could not set up beverage options')}finally{setSaving(false)}}
  ...
  const editItem=i=>{setEditingItem(i);setItemForm({name:i.name||'',description:i.description||'',category_public_id:i.category_public_id||'',base_price:i.base_price??'',image_url:i.image_url||'',is_active:i.is_active!==false,is_available:i.is_available!==false,modifier_group_public_ids:i.modifier_group_public_ids||[],unavailable_branch_public_ids:i.unavailable_branch_public_ids||[]});setTab('items')}
  const editGroup=g=>{setEditingGroup(g);setGroupForm({name:g.name||'',selection_type:g.selection_type||'single',is_required:!!g.is_required,min_selections:g.min_selections||0,max_selections:g.max_selections||1,is_active:g.is_active!==false,options:(g.options||[]).map(o=>({name:o.name||'',price_delta:o.price_delta||0,is_active:o.is_active!==false}))});setTab('groups')}
