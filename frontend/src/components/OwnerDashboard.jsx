...
function OrderAheadMenuBuilder({ API_BASE, user, business, branches, authFetch, onChanged, setMessage }) {
  const displayCurrency = business?.display_currency || 'PHP'
  const money = (value) => formatMoney(Number(value || 0), displayCurrency, { minimumFractionDigits:2, maximumFractionDigits:2 })
  const [tab,setTab]=useState('items')
  const [data,setData]=useState({categories:[],items:[],modifier_groups:[],branches:[]})
  const [loading,setLoading]=useState(true)
  const [saving,setSaving]=useState(false)
  const emptyItem={name:'',description:'',category_public_id:'',base_price:'',image_url:'',is_active:true,is_available:true,modifier_group_public_ids:[],excluded_inherited_modifier_group_public_ids:[],unavailable_branch_public_ids:[]}
  const [itemForm,setItemForm]=useState(emptyItem)
  const [editingItem,setEditingItem]=useState(null)
  const [categoryForm,setCategoryForm]=useState({name:'',description:'',is_active:true})
  const [editingCategory,setEditingCategory]=useState(null)
  const [groupForm,setGroupForm]=useState({name:'',selection_type:'single',is_required:false,min_selections:0,max_selections:1,is_active:true,applies_to_all:false,category_public_ids:[],options:[{name:'',price_delta:0,is_active:true}]})
  ...
  const loadMenu=async()=>{
    if(!slug)return
    setLoading(true)
    try{const r=await authFetch(`${API_BASE}/api/v1/business/${slug}/order-ahead/menu`,{cache:'no-store'});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.detail||'Could not load menu');setData({categories:j.categories||[],items:j.items||[],modifier_groups:j.modifier_groups||[],branches:j.branches||[]})}
    catch(e){setMessage?.(e.message||'Could not load menu')}
    finally{setLoading(false)}
  }
  useEffect(()=>{loadMenu()},[slug])
  const call=async(url,method,body)=>{setSaving(true);try{const r=await authFetch(url,{method,headers:{'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.detail||'Could not save');await loadMenu();onChanged?.();return true}catch(e){setMessage?.(e.message||'Could not save');return false}finally{setSaving(false)}}
  const saveCategory=async()=>{if(!categoryForm.name.trim())return;const body={...categoryForm,name:categoryForm.name.trim(),sort_order:editingCategory?.sort_order||0};const url=editingCategory?`${API_BASE}/api/v1/business/${slug}/order-ahead/categories/${editingCategory.public_id}`:`${API_BASE}/api/v1/business/${slug}/order-ahead/categories`;if(await call(url,editingCategory?'PATCH':'POST',body)){setCategoryForm({name:'',description:'',is_active:true});setEditingCategory(null)}}
  const saveGroup=async()=>{if(!groupForm.name.trim())return;const options=(groupForm.options||[]).filter(o=>o.name?.trim()).map((o,i)=>({...o,name:o.name.trim(),price_delta:Number(o.price_delta||0),sort_order:i,is_active:o.is_active!==false}));const body={...groupForm,name:groupForm.name.trim(),min_selections:Number(groupForm.is_required?Math.max(1,groupForm.min_selections||1):groupForm.min_selections||0),max_selections:groupForm.selection_type==='single'?1:Number(groupForm.max_selections||options.length||1),sort_order:editingGroup?.sort_order||0,options};const url=editingGroup?`${API_BASE}/api/v1/business/${slug}/order-ahead/modifier-groups/${editingGroup.public_id}`:`${API_BASE}/api/v1/business/${slug}/order-ahead/modifier-groups`;if(await call(url,editingGroup?'PATCH':'POST',body)){setGroupForm({name:'',selection_type:'single',is_required:false,min_selections:0,max_selections:1,is_active:true,applies_to_all:false,category_public_ids:[],options:[{name:'',price_delta:0,is_active:true}]});setEditingGroup(null)}}
  const saveItem=async()=>{if(!itemForm.name.trim())return;const body={...itemForm,name:itemForm.name.trim(),base_price:Number(itemForm.base_price||0),category_public_id:itemForm.category_public_id||null,sort_order:editingItem?.sort_order||0};const url=editingItem?`${API_BASE}/api/v1/business/${slug}/order-ahead/items/${editingItem.public_id}`:`${API_BASE}/api/v1/business/${slug}/order-ahead/items`;if(await call(url,editingItem?'PATCH':'POST',body)){setItemForm(emptyItem);setEditingItem(null)}}
  const chooseItemCategory=(categoryPublicId)=>setItemForm(f=>({...f,category_public_id:categoryPublicId,excluded_inherited_modifier_group_public_ids:[]}))
  const inheritedGroupIdsForCategory=(categoryPublicId)=>data.modifier_groups.filter(g=>g.applies_to_all||(g.category_public_ids||[]).includes(categoryPublicId)).map(g=>g.public_id)
  const groupName=id=>data.modifier_groups.find(g=>g.public_id===id)?.name||'Option'
  const quickSetupBeverageModifiers=async()=>{setSaving(true);try{const r=await authFetch(`${API_BASE}/api/v1/business/${slug}/order-ahead/beverage-modifiers/quick-setup`,{method:'POST'});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.detail||'Could not set up beverage options');await loadMenu();onChanged?.();setMessage?.(`Size + Sugar Level ready · assigned to ${j.beverage_categories_found||0} beverage categor${Number(j.beverage_categories_found||0)===1?'y':'ies'}`)}catch(e){setMessage?.(e.message||'Could not set up beverage options')}finally{setSaving(false)}}
  const uploadImage=async(file)=>{if(!file||!slug)return;setSaving(true);try{const sr=await authFetch(`${API_BASE}/api/v1/business/${slug}/cloudinary-signature?purpose=order_ahead_menu`,{method:'POST'});const sig=await sr.json();if(!sr.ok)throw new Error(sig.detail||'Could not prepare image upload');const form=new FormData();form.append('file',file);form.append('api_key',sig.api_key);form.append('timestamp',sig.timestamp);form.append('signature',sig.signature);form.append('upload_preset',sig.upload_preset);form.append('folder',sig.folder);const ur=await fetch(`https://api.cloudinary.com/v1_1/${sig.cloud_name}/image/upload`,{method:'POST',body:form});const up=await ur.json();if(!ur.ok)throw new Error(up.error?.message||'Image upload failed');setItemForm(f=>({...f,image_url:up.secure_url||up.url||''}))}catch(e){setMessage?.(e.message||'Image upload failed')}finally{setSaving(false)}}
  const editItem=i=>{setEditingItem(i);setItemForm({name:i.name||'',description:i.description||'',category_public_id:i.category_public_id||'',base_price:i.base_price??'',image_url:i.image_url||'',is_active:i.is_active!==false,is_available:i.is_available!==false,modifier_group_public_ids:i.direct_modifier_group_public_ids||[],excluded_inherited_modifier_group_public_ids:i.excluded_inherited_modifier_group_public_ids||[],unavailable_branch_public_ids:i.unavailable_branch_public_ids||[]});setTab('items')}
  const editGroup=g=>{setEditingGroup(g);setGroupForm({name:g.name||'',selection_type:g.selection_type||'single',is_required:!!g.is_required,min_selections:g.min_selections||0,max_selections:g.max_selections||1,is_active:g.is_active!==false,applies_to_all:!!g.applies_to_all,category_public_ids:g.category_public_ids||[],options:(g.options||[]).map(o=>({name:o.name||'',price_delta:o.price_delta||0,is_active:o.is_active!==false}))});setTab('groups')}
  const editCategory=c=>{setEditingCategory(c);setCategoryForm({name:c.name||'',description:c.description||'',is_active:c.is_active!==false});setTab('categories')}
  const del=(kind,id)=>{if(!window.confirm('Delete this '+kind.replace('-',' ')+'?'))return;call(`${API_BASE}/api/v1/business/${slug}/order-ahead/${kind}/${id}`,'DELETE')}
  const pill={border:'1px solid #cbd5e1',background:'#fff',borderRadius:999,padding:'7px 10px',fontSize:11,fontWeight:800,cursor:'pointer'}
  const fld={width:'100%',padding:'10px 11px',border:'1px solid #cbd5e1',borderRadius:9,boxSizing:'border-box'}
  if(loading)return <div style={{...styles.card,marginTop:14}}>Loading Menu Builder…</div>
  return <div style={{...styles.card,marginTop:14}}>
    <div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',flexWrap:'wrap'}}><div><div style={{fontSize:11,fontWeight:900,color:'#0f766e'}}>MENU BUILDER</div><h3 style={{margin:'4px 0'}}>Build the menu customers order from</h3><p style={{margin:0,fontSize:12,color:'#64748b'}}>Owner controls content. Loyalty Tree Super Admin controls the customer-facing layout.</p></div><div style={{display:'flex',gap:6,flexWrap:'wrap'}}>{[['items','Items'],['categories','Categories'],['groups','Option Groups'],['preview','Preview']].map(([k,l])=><button key={k} onClick={()=>setTab(k)} style={{...pill,...(tab===k?{background:'#0f766e',color:'#fff',borderColor:'#0f766e'}:{})}}>{l}</button>)}</div></div>

    {tab==='categories'&&<div style={{marginTop:16,display:'grid',gridTemplateColumns:'minmax(250px,.9fr) minmax(0,1.4fr)',gap:14}}><div style={{background:'#f8fafc',borderRadius:12,padding:12}}><b>{editingCategory?'Edit category':'Add category'}</b><input style={{...fld,marginTop:10}} placeholder="Coffee" value={categoryForm.name} onChange={e=>setCategoryForm({...categoryForm,name:e.target.value})}/><textarea style={{...fld,marginTop:8,minHeight:70}} placeholder="Optional description" value={categoryForm.description||''} onChange={e=>setCategoryForm({...categoryForm,description:e.target.value})}/><label style={{fontSize:12,display:'flex',gap:7,marginTop:8}}><input type="checkbox" checked={categoryForm.is_active} onChange={e=>setCategoryForm({...categoryForm,is_active:e.target.checked})}/> Visible</label><button disabled={saving} onClick={saveCategory} style={{...styles.submitBtn,padding:10}}>{editingCategory?'Save category':'Add category'}</button>{editingCategory&&<button onClick={()=>{setEditingCategory(null);setCategoryForm({name:'',description:'',is_active:true})}} style={{...styles.addBtn,marginTop:7}}>Cancel</button>}</div><div>{data.categories.length?data.categories.map(c=><div key={c.public_id} style={{display:'flex',justifyContent:'space-between',gap:10,padding:'11px 0',borderBottom:'1px solid #e2e8f0'}}><div><b>{c.name}</b>{c.is_active===false&&<span style={{fontSize:10,color:'#b45309'}}> · Hidden</span>}<div style={{fontSize:11,color:'#64748b'}}>{data.items.filter(i=>i.category_public_id===c.public_id).length} items</div></div><div><button style={pill} onClick={()=>editCategory(c)}>Edit</button> <button style={pill} onClick={()=>del('categories',c.public_id)}>Delete</button></div></div>):<div style={{color:'#94a3b8'}}>No categories yet.</div>}</div></div>}

    {tab==='groups'&&<><div style={{marginTop:16,background:'#ecfeff',border:'1px solid #a5f3fc',borderRadius:12,padding:12,display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',flexWrap:'wrap'}}><div><b>Reusable option groups</b><div style={{fontSize:11,color:'#155e75',marginTop:3}}>Assign each group to categories instead of attaching it product-by-product. Example: Size + Sugar Level → Coffee and Non-Coffee; Spice Level → Food; Side Dish → Meals.</div></div><button disabled={saving} onClick={quickSetupBeverageModifiers} style={{...styles.submitBtn,padding:'9px 12px',marginTop:0,width:'auto'}}>Quick add Size + Sugar</button></div><div style={{marginTop:12,display:'grid',gridTemplateColumns:'minmax(280px,1fr) minmax(0,1.2fr)',gap:14}}><div style={{background:'#f8fafc',borderRadius:12,padding:12}}><b>{editingGroup?'Edit option group':'Add option group'}</b><input style={{...fld,marginTop:10}} placeholder="Size" value={groupForm.name} onChange={e=>setGroupForm({...groupForm,name:e.target.value})}/><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginTop:8}}><select style={fld} value={groupForm.selection_type} onChange={e=>setGroupForm({...groupForm,selection_type:e.target.value})}><option value="single">Choose one</option><option value="multiple">Choose multiple</option></select><label style={{fontSize:12,display:'flex',gap:7,alignItems:'center'}}><input type="checkbox" checked={groupForm.is_required} onChange={e=>setGroupForm({...groupForm,is_required:e.target.checked})}/> Required</label></div><div style={{fontSize:11,fontWeight:800,color:'#64748b',marginTop:12}}>OPTIONS</div>{(groupForm.options||[]).map((opt,idx)=><div key={idx} style={{display:'grid',gridTemplateColumns:'1fr 90px 30px',gap:6,marginTop:6}}><input style={fld} placeholder="Large" value={opt.name} onChange={e=>setGroupForm({...groupForm,options:groupForm.options.map((x,i)=>i===idx?{...x,name:e.target.value}:x)})}/><input style={fld} type="number" step="0.01" placeholder={`+${currencySymbol(displayCurrency)}`} value={opt.price_delta} onChange={e=>setGroupForm({...groupForm,options:groupForm.options.map((x,i)=>i===idx?{...x,price_delta:e.target.value}:x)})}/><button style={{...pill,padding:4}} onClick={()=>setGroupForm({...groupForm,options:groupForm.options.filter((_,i)=>i!==idx)})}>×</button></div>)}<button style={{...styles.addBtn,marginTop:8}} onClick={()=>setGroupForm({...groupForm,options:[...groupForm.options,{name:'',price_delta:0,is_active:true}]})}>+ Add option</button><div style={{fontSize:11,fontWeight:800,color:'#64748b',marginTop:14}}>APPLIES TO</div><label style={{display:'flex',gap:7,fontSize:12,marginTop:7,fontWeight:800}}><input type="checkbox" checked={!!groupForm.applies_to_all} onChange={e=>setGroupForm({...groupForm,applies_to_all:e.target.checked,category_public_ids:e.target.checked?[]:groupForm.category_public_ids})}/> All menu items</label>{!groupForm.applies_to_all&&<div style={{marginTop:5}}>{data.categories.length?data.categories.map(c=><label key={c.public_id} style={{display:'flex',gap:7,fontSize:12,marginTop:5}}><input type="checkbox" checked={(groupForm.category_public_ids||[]).includes(c.public_id)} onChange={e=>setGroupForm({...groupForm,category_public_ids:e.target.checked?[...(groupForm.category_public_ids||[]),c.public_id]:(groupForm.category_public_ids||[]).filter(x=>x!==c.public_id)})}/>{c.name}</label>):<div style={{fontSize:11,color:'#94a3b8',marginTop:5}}>Create categories first, or choose All menu items.</div>}</div>}<div style={{fontSize:10,color:'#64748b',marginTop:8}}>Items in selected categories inherit this group automatically. You can exclude it from a specific item when editing that item.</div><button disabled={saving} onClick={saveGroup} style={{...styles.submitBtn,padding:10}}>{editingGroup?'Save group':'Add option group'}</button>{editingGroup&&<button onClick={()=>{setEditingGroup(null);setGroupForm({name:'',selection_type:'single',is_required:false,min_selections:0,max_selections:1,is_active:true,applies_to_all:false,category_public_ids:[],options:[{name:'',price_delta:0,is_active:true}]})}} style={{...styles.addBtn,marginTop:7}}>Cancel</button>}</div><div>{data.modifier_groups.length?data.modifier_groups.map(g=>{const applies=g.applies_to_all?'All menu items':(g.category_public_ids||[]).map(id=>data.categories.find(c=>c.public_id===id)?.name).filter(Boolean).join(' · ')||'Not assigned';return <div key={g.public_id} style={{padding:'11px 0',borderBottom:'1px solid #e2e8f0'}}><div style={{display:'flex',justifyContent:'space-between',gap:8}}><div><b>{g.name}</b><div style={{fontSize:11,color:'#64748b'}}>{g.is_required?'Required · ':''}{g.selection_type==='multiple'?'Multiple choice':'Single choice'} · {(g.options||[]).length} options</div><div style={{fontSize:11,color:applies==='Not assigned'?'#b45309':'#0f766e',marginTop:3}}>Applies to: {applies}</div></div><div><button style={pill} onClick={()=>editGroup(g)}>Edit</button> <button style={pill} onClick={()=>del('modifier-groups',g.public_id)}>Delete</button></div></div><div style={{fontSize:11,color:'#475569',marginTop:6}}>{(g.options||[]).map(opt=>`${opt.name}${Number(opt.price_delta)?` (+${money(opt.price_delta)})`:''}`).join(' · ')}</div></div>}):<div style={{color:'#94a3b8'}}>No option groups yet. Create reusable groups like Size, Milk, Add-ons, Spice Level, or Side Dish.</div>}</div></div></>}


    {tab==='items'&&<div style={{marginTop:16,display:'grid',gridTemplateColumns:'minmax(300px,1fr) minmax(0,1.2fr)',gap:14}}><div style={{background:'#f8fafc',borderRadius:12,padding:12}}><b>{editingItem?'Edit menu item':'Add menu item'}</b><input style={{...fld,marginTop:10}} placeholder="Iced Latte" value={itemForm.name} onChange={e=>setItemForm({...itemForm,name:e.target.value})}/><textarea style={{...fld,marginTop:8,minHeight:70}} placeholder="Short description" value={itemForm.description||''} onChange={e=>setItemForm({...itemForm,description:e.target.value})}/><div style={{display:'grid',gridTemplateColumns:'1fr 120px',gap:8,marginTop:8}}><select style={fld} value={itemForm.category_public_id} onChange={e=>chooseItemCategory(e.target.value)}><option value="">No category</option>{data.categories.map(c=><option key={c.public_id} value={c.public_id}>{c.name}</option>)}</select><input style={fld} type="number" min="0" step="0.01" placeholder="Price" value={itemForm.base_price} onChange={e=>setItemForm({...itemForm,base_price:e.target.value})}/></div><div style={{marginTop:9}}><label style={{fontSize:11,fontWeight:800,color:'#64748b'}}>PRODUCT PHOTO</label><input type="file" accept="image/*" onChange={e=>uploadImage(e.target.files?.[0])} style={{display:'block',marginTop:5,fontSize:11}}/>{itemForm.image_url&&<img src={itemForm.image_url} alt="" style={{width:90,height:70,objectFit:'cover',borderRadius:9,marginTop:7}}/>}</div>{(()=>{const inherited=inheritedGroupIdsForCategory(itemForm.category_public_id);return <><div style={{fontSize:11,fontWeight:800,color:'#64748b',marginTop:12}}>INHERITED FROM CATEGORY</div>{inherited.length?inherited.map(id=><label key={id} style={{display:'flex',gap:7,fontSize:12,marginTop:5,alignItems:'center'}}><input type="checkbox" checked={!(itemForm.excluded_inherited_modifier_group_public_ids||[]).includes(id)} onChange={e=>setItemForm({...itemForm,excluded_inherited_modifier_group_public_ids:e.target.checked?(itemForm.excluded_inherited_modifier_group_public_ids||[]).filter(x=>x!==id):[...(itemForm.excluded_inherited_modifier_group_public_ids||[]),id]})}/>{groupName(id)} <span style={{fontSize:10,color:'#0f766e'}}>inherited</span></label>):<div style={{fontSize:11,color:'#94a3b8',marginTop:5}}>No category option groups apply to this item.</div>}<div style={{fontSize:11,fontWeight:800,color:'#64748b',marginTop:12}}>ITEM-SPECIFIC OPTIONS</div>{data.modifier_groups.filter(g=>!inherited.includes(g.public_id)).map(g=><label key={g.public_id} style={{display:'flex',gap:7,fontSize:12,marginTop:5}}><input type="checkbox" checked={(itemForm.modifier_group_public_ids||[]).includes(g.public_id)} onChange={e=>setItemForm({...itemForm,modifier_group_public_ids:e.target.checked?[...(itemForm.modifier_group_public_ids||[]),g.public_id]:(itemForm.modifier_group_public_ids||[]).filter(x=>x!==g.public_id)})}/>{g.name}</label>)}</>})()}<div style={{fontSize:11,fontWeight:800,color:'#64748b',marginTop:12}}>BRANCH AVAILABILITY</div>{(data.branches.length?data.branches:branches).map(branch=><label key={branch.public_id} style={{display:'flex',gap:7,fontSize:12,marginTop:5}}><input type="checkbox" checked={!itemForm.unavailable_branch_public_ids.includes(branch.public_id)} onChange={e=>setItemForm({...itemForm,unavailable_branch_public_ids:e.target.checked?itemForm.unavailable_branch_public_ids.filter(x=>x!==branch.public_id):[...itemForm.unavailable_branch_public_ids,branch.public_id]})}/>{branch.name}</label>)}<div style={{display:'flex',gap:12,marginTop:10,fontSize:12}}><label><input type="checkbox" checked={itemForm.is_active} onChange={e=>setItemForm({...itemForm,is_active:e.target.checked})}/> Show on menu</label><label><input type="checkbox" checked={itemForm.is_available} onChange={e=>setItemForm({...itemForm,is_available:e.target.checked})}/> Available</label></div><button disabled={saving} onClick={saveItem} style={{...styles.submitBtn,padding:10}}>{editingItem?'Save item':'Add item'}</button>{editingItem&&<button onClick={()=>{setEditingItem(null);setItemForm(emptyItem)}} style={{...styles.addBtn,marginTop:7}}>Cancel</button>}</div><div>{data.items.length?data.items.map(i=>{const effective=(i.effective_modifier_group_public_ids||i.modifier_group_public_ids||[]).map(groupName);return <div key={i.public_id} style={{display:'grid',gridTemplateColumns:'58px 1fr auto',gap:10,alignItems:'center',padding:'10px 0',borderBottom:'1px solid #e2e8f0'}}>{i.image_url?<img src={i.image_url} alt="" style={{width:58,height:48,objectFit:'cover',borderRadius:8}}/>:<div style={{width:58,height:48,borderRadius:8,background:'#e2e8f0'}}/>}<div><b>{i.name}</b> <span style={{fontSize:11,fontWeight:800}}>{money(i.base_price)}</span><div style={{fontSize:11,color:i.is_available===false?'#b91c1c':'#64748b'}}>{i.is_active===false?'Hidden':i.is_available===false?'Sold out':data.categories.find(c=>c.public_id===i.category_public_id)?.name||'Uncategorized'}</div><div style={{fontSize:10,color:effective.length?'#0f766e':'#94a3b8',marginTop:3}}>Options: {effective.length?effective.join(' · '):'None'}</div></div><div><button style={pill} onClick={()=>editItem(i)}>Edit</button> <button style={pill} onClick={()=>del('items',i.public_id)}>Delete</button></div></div>}):<div style={{color:'#94a3b8'}}>No menu items yet.</div>}</div></div>}


    {tab==='preview'&&<div style={{marginTop:16,maxWidth:390,marginLeft:'auto',marginRight:'auto',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:22,padding:14}}><div style={{fontSize:11,color:'#64748b',marginBottom:8}}>OWNER CONTENT PREVIEW · FINAL STYLE IS CONTROLLED BY SUPER ADMIN</div><div style={{display:'flex',gap:7,overflow:'auto',marginBottom:10}}>{data.categories.slice(0,5).map(c=><span key={c.public_id} style={{...pill,whiteSpace:'nowrap'}}>{c.name}</span>)}</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>{data.items.filter(i=>i.is_active!==false).slice(0,6).map(i=><div key={i.public_id} style={{background:'#fff',borderRadius:12,overflow:'hidden',border:'1px solid #e2e8f0'}}>{i.image_url?<img src={i.image_url} alt="" style={{width:'100%',height:90,objectFit:'cover'}}/>:<div style={{height:90,background:'#e2e8f0'}}/>}<div style={{padding:9}}><b style={{fontSize:12}}>{i.name}</b><div style={{fontSize:11,marginTop:5}}>{money(i.base_price)}</div></div></div>)}</div></div>}
  </div>
}
...
function OwnerDashboard({ API_BASE, user, onLogout }) {
  ...
  const canExitSetupGuide = cardSetUp
  ...
  const openEditCustomer = (c) => {
    setEditForm({
      ...
      tier_stamp_count: c.tier_stamp_count ?? (c.tier_progression_type === 'stamps' ? (c.stamp_count ?? 0) : 0),
  ...
  const fetchMemberHistory = async (customerPublicId) => {
    ...
    try {
      if (program?.card_type === 'hybrid') {
        ...
        const tierEnabled = program?.hybrid_tier_enabled === true
        const requests = [
          ...
          ...(tierEnabled ? [authFetch(`${API_BASE}/api/v1/business/${user.business_slug}/customers/${customerPublicId}/vip-history`)] : []),
        ]
        const responses = await Promise.all(requests)
        const loyaltyData = await responses[0].json().catch(() => [])
        const membershipData = await responses[1].json().catch(() => [])
        const benefitData = await responses[2].json().catch(() => [])
        const tierData = tierEnabled ? await responses[3].json().catch(() => []) : []
        const combined = [
          ...
          ...(Array.isArray(tierData) ? tierData.map(x => ({...x, hybrid_source:'tier', activity_type:x.activity_type || x.action || 'tier_activity'})) : []),
        ...
        const combined = [
          ...(vipRes.ok && Array.isArray(vipData) ? vipData.map(x => ({...x, tier_stamp_source:'vip'})) : []),
          ...(stampRes.ok && Array.isArray(stampData) ? stampData.map(x => ({...x, tier_stamp_source:'stamp'})) : []),
  ...
  const hybridTierEnabled = isHybridCard && program?.hybrid_tier_enabled === true
  const hybridTierUsesStamps = hybridTierEnabled && program?.hybrid_tier_progression_type !== 'points'
  const hybridTierUsesPoints = hybridTierEnabled && !hybridTierUsesStamps
  ...
  const programHasTier = isVipCard || hybridTierEnabled
  ...
  const totalTierStamps = customers.reduce((sum, c) => sum + (c.tier_stamp_count ?? (isVipCard && vipUsesStamps ? (c.stamp_count || 0) : 0)), 0)
                  ...
                  <div style={styles.customerInfo}>
                    ...
                    {isHybridCard ? (
                      ...
                      <>
                        ...
                        <p style={styles.lastStampedText}>{vipUsesStamps ? `${c.tier_stamp_count ?? c.stamp_count ?? 0} Tier stamps` : `${c.vip_points||0} Tier points`}{c.vip_next_tier?` · ${Math.max(0,c.vip_next_tier.threshold-(vipUsesStamps?(c.tier_stamp_count ?? c.stamp_count ?? 0):(c.vip_points||0)))} ${vipUsesStamps?'stamps':'points'} to ${c.vip_next_tier.name}`:' · Highest tier'}</p>
                      </>
                    ) : isMembershipCard ? (
                      <>
          ...
          <div style={styles.section}>
            ...
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(2,minmax(0,1fr))',gap:12}}>
              ...
              <div style={styles.card}>
                ...
                <p style={{margin:'0 0 10px',fontSize:12,color:'#64748b'}}>{orderAheadSetup?.menu?.categories_count || 0} categories · reusable option groups · branch availability · sold-out control.</p>
                <span style={{display:'inline-block',background:'#ecfdf5',color:'#047857',padding:'6px 9px',borderRadius:999,fontSize:11,fontWeight:800}}>Menu Builder ready</span>
              </div>
            ...
            <OrderAheadMenuBuilder API_BASE={API_BASE} user={user} business={business} branches={branches} authFetch={authFetch} onChanged={loadData} setMessage={setMessage} />
            ...
            <div style={{...styles.card,marginTop:14}}>
              ...
              <div style={{borderTop:'1px solid #e2e8f0',marginTop:18,paddingTop:16}}>
                <div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'flex-start',flexWrap:'wrap'}}>
                  ...
                  <span style={{background:'#f8fafc',color:'#64748b',padding:'6px 9px',borderRadius:999,fontSize:10.5,fontWeight:800}}>Business local time</span>
                </div>
                ...
                {orderAheadDisplayedOrders.map(order=>{
                  ...
                  return <div key={order.public_id} style={{background:'#fff',border:`1px solid ${urgency?.border || '#e2e8f0'}`,borderRadius:13,padding:12,boxShadow:urgency?'0 6px 18px rgba(15,23,42,.07)':'0 3px 10px rgba(15,23,42,.03)'}}>
                    <div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'flex-start'}}>
                      ...
                      <strong style={{fontSize:14}}>{formatMoney(order.total||0,businessCurrency,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong>
                    </div>
          ...
          <div style={styles.programTab}>
            ...
            {isMembershipCard && (
              <div style={{...styles.card, marginTop: 18}}>
                ...
                />
                <label style={styles.label}>Default price ({currencySymbol(businessCurrency)})</label>
                <input
                  style={styles.input}
                  type="number"
                  min="0"
              ...
              <div style={styles.cardBody}>
                ...
                {isHybridCard ? (
                  <div style={styles.cardProgress}>
                    ...
                    {hybridTierEnabled && <div style={{marginTop:10,paddingTop:9,borderTop:'1px solid rgba(255,255,255,.24)',textAlign:'left'}}>
                  ...
                  <div style={styles.cardProgress}>
                    ...
                    <p style={{fontSize:14,color:'white',fontWeight:800}}>{vipUsesStamps ? `Tier Stamps: ${selectedCustomer.tier_stamp_count ?? selectedCustomer.stamp_count ?? 0}` : `Tier Points: ${selectedCustomer.vip_points||0}`}</p>
                    {selectedCustomer.vip_next_tier&&<p style={{fontSize:12,color:'rgba(255,255,255,.8)'}}>{Math.max(0,selectedCustomer.vip_next_tier.threshold-(vipUsesStamps?(selectedCustomer.tier_stamp_count ?? selectedCustomer.stamp_count ?? 0):(selectedCustomer.vip_points||0)))} {vipUsesStamps?'stamps':'points'} to {selectedCustomer.vip_next_tier.name}</p>}
                ...
                {memberHistoryLoading ? <p style={{color:'#64748b'}}>Loading activity...</p> : memberHistory.length === 0 ? (
                  ...
                  <div style={{maxHeight:320, overflowY:'auto', background:'white', borderRadius:12, padding:'0 12px'}}>
                    {memberHistory.map((item, i) => {
                      ...
                      const isTierStampEvent = isVipCard && item.tier_stamp_source === 'stamp' && !isCouponEvent
                      const isHybridTierEvent = isHybridCard && item.hybrid_source === 'tier' && !isCouponEvent
                      ...
                      const title = isHybridCard && item.hybrid_source === 'benefit'
                        ...
                        : isHybridTierEvent
                        ? hybridTierUsesStamps
                          ? `${Number(item.points_delta || 0) >= 0 ? '+' : ''}${Number(item.points_delta || 0)} Tier stamp${Math.abs(Number(item.points_delta || 0)) === 1 ? '' : 's'}`
                          : `${Number(item.points_delta || 0) >= 0 ? '+' : ''}${Number(item.points_delta || 0)} Tier pts`
                      ...
                      const tierChanged = (isVipCard || isHybridTierEvent) && item.old_tier && item.new_tier && item.old_tier !== item.new_tier
                      ...
                      return <div key={item.id || i} style={{display:'grid',gridTemplateColumns:'12px 1fr',gap:10,padding:'12px 0',borderBottom:i===memberHistory.length-1?'none':'1px solid #e2e8f0'}}>
                        <div style={{
                          ...
                          background:(
                            ...
                            (isTierStampEvent && item.activity_type === 'adjustment' && Number(item.delta || 0) < 0) ||
                        ...
                        <div>
                          ...
                          {isTierStampEvent && item.activity_type === 'adjustment' && (
                            <>
                              <div style={{fontSize:13,color:Number(item.delta || 0) < 0 ? '#b91c1c' : '#166534',fontWeight:700,marginTop:3}}>
                                {item.old_count ?? 0} stamps → {item.new_count ?? 0} stamps
                              </div>
                          ...
                          {(isPointsCard || (isHybridCard && hybridUsesPoints && item.hybrid_source === 'loyalty')) && !isCouponEvent && item.amount_spent_pesos != null && <div style={{fontSize:13,color:'#475569',marginTop:3}}>{formatMoney(item.amount_spent_pesos,businessCurrency)} spent</div>}
                          ...
                          {tierChanged && <div style={{fontSize:13,fontWeight:700,color:'#b45309',marginTop:3}}>🏆 Tier: {item.old_tier} → {item.new_tier}</div>}
                          {isHybridTierEvent && (item.amount_spent != null || item.points_balance != null) && (
                            <div style={{fontSize:13,color:'#475569',marginTop:3}}>
                              {item.amount_spent != null ? `${formatMoney(item.amount_spent,businessCurrency)} tier-qualifying spend` : null}
                              {item.amount_spent != null && item.points_balance != null ? ' • ' : null}
                              {item.points_balance != null ? `${item.points_balance} Tier ${hybridTierUsesStamps ? 'stamps' : 'points'} after` : null}
                            </div>
                          )}
                          {isHybridTierEvent && item.note && <div style={{fontSize:13,color:'#334155',whiteSpace:'pre-wrap',marginTop:6,padding:8,background:'#fff7ed',borderRadius:8}}>{item.note}</div>}
                          {isVipCard && (item.amount_spent != null || item.points_balance != null) && (
                            <div style={{fontSize:13,color:'#475569',marginTop:3}}>
                              {item.amount_spent != null ? `${formatMoney(item.amount_spent,businessCurrency)} spent` : null}
                              {item.amount_spent != null && item.points_balance != null ? ' • ' : null}
            ...
            <form onSubmit={saveCustomer}>
              ...
              {isHybridCard ? (
                <>
                  ...
                  {hybridUsesPoints ? (<>
                    <label style={styles.label}>Reward Points Balance</label>
                    ...
                    <label style={styles.label}>Reward Stamps {program?.stamp_goal ? `(of ${program.stamp_goal} for a reward)` : ''}</label>
                  ...
                  {hybridTierEnabled && <div style={{padding:12,border:'1px solid #fde68a',background:'#fffbeb',borderRadius:12,margin:'4px 0 14px'}}>
                    <strong style={{display:'block',color:'#92400e',marginBottom:8}}>👑 Tier Progress</strong>
                    {hybridTierUsesStamps ? <>
                      <label style={styles.label}>Tier Stamps</label>
                      <input style={styles.input} type="number" min="0" value={editForm.tier_stamp_count ?? 0} onChange={e=>setEditForm({...editForm,tier_stamp_count:e.target.value})}/>
                    </> : <>
                      <label style={styles.label}>Tier Points</label>
                ...
                <>
                  {vipUsesStamps ? <>
                    <label style={styles.label}>Tier stamps</label><input style={styles.input} type='number' min='0' value={editForm.tier_stamp_count??editForm.stamp_count??0} onChange={e=>setEditForm({...editForm,tier_stamp_count:e.target.value})}/>
                  </> : <>
