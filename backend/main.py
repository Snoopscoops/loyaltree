...
async def showroom_page(business_public_id: str):
    try:
        ...
        hero_html = (
            ...
            '<div><strong>' + str(sum(1 for x in vehicles if x.get('status') == 'available')) + '</strong><span>Available now</span></div>'
            '<div><strong>5 min</strong><span>Fast initial approval</span></div>'
            '<div><strong>Trade-in</strong><span>Upgrade or downgrade</span></div>'
            '</div>'
            '</div>'
